...
export default function Projects() {
    ...
    useEffect(() => {
        async function getProjects() {
            setLoading(true)
            const q = query(collection(db, "myprojects"));
    ...
    return (
        <div className='w-[90%] lg:w-[80%] xl:w-[70%] mx-auto mb-32' id='projects'>
            {!loading && (
                <div className='flex flex-col'>
                    <h3 className='text-xl font-medium border-b-2 border-[#007ced] self-start pb-2 mb-12'>My Projects</h3>
                            ...
                            return (
                                <div key={v4()} className='bg-[#333] rounded-md shadow-md border-[2px] border-[#444] text-center py-4 px-6'>
                                    ...
                                    <div>
                                        ...
                                        onClick={() => window.location.href = project.sourceUrl}>Source</button>
                                        <button className='bg-[#3a3939] py-2 px-3 rounded-md shadow-sm m-1 hover:bg-[#424242] transiton ease-in-out duration-200'
                                        onClick={() => window.location.href = project.previewUrl}>Live Demo</button>
                                    </div>
                                </div>
                            )
                        })}
